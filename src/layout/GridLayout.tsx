...
// Pre-create grid placeholder array to avoid recreating on each render
const EDIT_MODE_GRID_CELLS = Array.from({ length: 24 });

// Memoized widget renderer to prevent unnecessary re-renders
const WidgetRenderer = memo<{ widget: WidgetInstance }>(({ widget }) => {
  switch (widget.type) {
...
export const GridLayout: React.FC = () => {
  ...
  return (
    <div className="w-full h-full relative overflow-y-auto overflow-x-hidden custom-scrollbar">
      <LayoutGroup>
        {/* Grid Container */}
        <div className="grid grid-cols-12 gap-5 lg:gap-6 auto-rows-[minmax(120px,auto)] max-w-[1600px] mx-auto pt-4 pb-24">
